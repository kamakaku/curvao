...
import { ImageBackground, StyleSheet, Text, View, Dimensions, ScrollView, Pressable } from 'react-native';
...
export function MatchHeroDetail({ card, wantedState }: { card: UserCard; wantedState?: { isOwned: boolean; isWanted: boolean; onToggleWanted: () => void; }; }) {
  const insets = useSafeAreaInsets();
  const [currentCard, setCurrentCard] = useState(card);
  const [actionError, setActionError] = useState<string | undefined>();
  const [isOptionsOpen, setIsOptionsOpen] = useState(false);
  const { match, homeClub, awayClub, stadium } = getCardRelations(currentCard);
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.wrapper}>
          <ImageBackground source={stadiumBg} style={[styles.background, { height: HERO_HEIGHT }]} imageStyle={styles.backgroundImage}>
            ...
            <View style={[styles.content, { paddingTop: Math.max(24, insets.top + 10) }]}>
              ...
              <View style={styles.mainInfoRow}>
                <View style={styles.matchTextSection}>
                  ...
                  <View style={styles.artifactMetaRow}>
                    ...
                    <Text style={styles.artifactMetaText}>{stadium?.name?.toUpperCase() || currentCard.stadiumName?.toUpperCase() || 'OLYMPIASTADION BERLIN'}</Text>
                  </View>
        ...
        <View style={styles.lowerContent}>
          ...
          <MatchBiographyBoxes card={currentCard} />
