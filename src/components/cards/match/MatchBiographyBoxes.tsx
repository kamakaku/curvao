...
export function MatchBiographyBoxes({ card }: { card: UserCard }) {
  const { match, stadium } = getCardRelations(card);

  const kickoffDate = match?.kickoffAt ? new Date(match.kickoffAt).toLocaleDateString('de-DE') : '—';
  const kickoffTime = match?.kickoffAt ? new Date(match.kickoffAt).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }) : '—';

  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>MATCH INFOS</Text>
      <View style={styles.grid}>
        ...
        <InfoBox 
          icon="time-outline" 
          label="UHRZEIT" 
          value={`${kickoffTime} UHR`} 
        />
        <InfoBox 
          icon="location-outline" 
          label="STADION" 
          value={stadium?.name?.split(' ')[0].toUpperCase() || card.stadiumName?.toUpperCase() || '—'} 
        />
        <InfoBox 
          icon="people-outline" 
          label="ZUSCHAUER" 
